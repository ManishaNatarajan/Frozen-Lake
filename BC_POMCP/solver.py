...
class POMCPSolver:
    ...
    def rollout_helper(self, augmented_state, robot_action, human_action, depth):
        ...
        if self.env.isTerminal(next_augmented_state[:len(self.env.world_state)]):
        ...
        # return self.env.reward(augmented_state, robot_action) + self.env.gamma * self.rollout_helper(next_augmented_state,
        #                                                                                              next_robot_action,
        #                                                                                              next_human_action, depth + 1)
    ...
    def simulate(self, augmented_state, action_node, depth):
        ...
        if self.env.isTerminal(second_augmented_state[:len(self.env.world_state)]):
            return self.env.final_reward(second_augmented_state)

        # If the robot action node is not in the tree, then returns the rollout value
        if robot_action_node == "empty":
            # rollout_value = self.rollout(augmented_state, robot_action, action_node, depth) #update state first?
            rollout_value = self.rollout(second_augmented_state, robot_action, action_node, depth)
            # print("Rollout value at position {} with robot action {}:".format(world_state[0], robot_action), rollout_value)
        ...
        # print("Current state {} depth {} reward: ".format(second_augmented_state[0], depth), curr_reward)
        value = curr_reward + self.env.gamma * self.simulate(
        ...
        # Also I'm not sure if we need to maintain these theta based statistics --> really depends on the human policy during rollout
        next_action_node.update_visited()  # TODO: check this... They are passing in theta here
        next_action_node.update_value(value)  # TODO: check this... They are passing in theta here
