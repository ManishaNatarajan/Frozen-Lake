...
from belief_update.frozenlake_map import MAPS, FOG, HUMAN_ERR, ROBOT_ERR
...
class FrozenLakeEnv:
    ...
    * v0: Initial version release
    ...
    def __init__(
        ...
        # pygame utils
        # self.window_size = (min(64 * ncol, 512), min(64 * nrow, 512) + 128)
    ...
    def augmented_state_transition(self, current_augmented_state, robot_action, human_action):
        ...
        # Latent states
        human_trust = current_augmented_state[6]
        human_capability = current_augmented_state[7]
        ...
        next_augmented_state = [next_world_state[0], next_world_state[1], next_world_state[2],
                                ...
                                next_human_trust, next_human_capability]

        return next_augmented_state

    def world_state_transition(self, current_world_state, robot_action, human_action):
        # world_state[1] is the last human position, it will be updated if human takes an action but will not if it's robot's turn.
        ...
        if human_action:
            ...
            # next_position = self.step(position, last_human_position, None, human_direction)
            # next_human_slippery, next_robot_slippery = self.detect_slippery_region(next_position, human_slippery, robot_slippery)
            ...
            # next_position = self.step(position, last_human_position, robot_action, None)
            # next_human_slippery, next_robot_slippery = self.detect_slippery_region(next_position, human_slippery, robot_slippery)
            ...
            # if robot_action[0] == 2:
            #     next_world_state = [next_position, last_human_position, next_human_slippery, next_robot_slippery]
            # else:
            next_world_state = [next_position, position, next_human_slippery, next_robot_slippery, next_human_err, next_robot_err]
    ...
    def get_rollout_observation(self, current_augmented_state, robot_action):
        ...
        current_human_capability = current_augmented_state[7]
        ...
        # Take over control
            ...
            if len(shortest_path) < 2:
                ...
                while len(actions) > 0 and (
                    ...
                    robot_action = actions.pop()
                    next_s = self.move(s_previous, robot_action)
                    if len(actions) == 0 and (self.desc[next_s // self.ncol, next_s % self.ncol] in b'HS' or next_s == s_previous):
                        # robot_action = best_action
                        return 0, None
                        # print("No action")
