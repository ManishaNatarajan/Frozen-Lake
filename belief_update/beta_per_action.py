...
def create_init_belief(num_particles=500):
    particle_set = []
    beta_params = np.array([[8, 3], [8, 4], [8, 5], [8, 4], [8, 4]])
...
def belief_update(particle_set, curr_robot_action, curr_human_action, get_counts=True):
    ...
    for i in sample_idx:
        ...
        acceptance_prob = np.random.beta(beta_params[curr_robot_action][0], beta_params[curr_robot_action][1])
        ...
        # Update beta params
        if accept:
            beta_params[curr_robot_action][0] += 1
        else:
            beta_params[curr_robot_action][1] += 1
